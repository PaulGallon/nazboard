...
describe("status payload", () => {
  it("returns fixture-backed JSON shape", async () => {
    ...
    try {
      ...
      assert.equal(status.disk_health.state, "healthy")
      ...
      assert.equal(status.disk_health.disks[0].pool_name, "storage01")
      assert.equal(status.disk_health.disks[0].vdev_name, "mirror-0")
      ...
      assert.equal(status.disk_health.disks[1].pool_name, "storage01")
      assert.equal(status.disk_health.disks[1].vdev_name, "mirror-4")
